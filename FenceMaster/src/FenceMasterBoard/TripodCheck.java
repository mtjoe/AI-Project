...
/**
 * Contains algorithm to check whether there are any tripods in the given board.
 * 
...
public class TripodCheck implements CheckLogic{
	...
	*/
	private Position checkCurrent(Position neighbors, Position current) {
		...
		if (!this.visited.contains(neighbors)) {
			...
			if (neighbors.isEdge && neighbors.isNonCorner) {
				...
				/*
				* If it is in the same other side of the board, check the 
				* queue
				*/
	...
	/**
	 * @return The Player that wins the game with Tripod, or null if no winner 
	 * is found
	 */
	public Player check() {	
